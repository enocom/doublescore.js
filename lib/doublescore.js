...
findByComparison = function(coll, memo, iterator, comparator) {
  var comparison,
      compare = function(left, right, comparator) {
        return comparator.call(nullContext, left, right);
      };
  ...
  each(coll, function(value) {
    if (iterator) {
      ...
      comparison = compare(a, b, comparator);
    } else {
      comparison = compare(value, memo, comparator);
    }
