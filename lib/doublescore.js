...
__.sortBy = function(coll, method) {
  return coll.sort(function(a, b) {
    ...
    return method.call(nullContext, a) - method.call(nullContext, b);
  });
};

function getRandomIndex(coll) {
  return Math.floor(Math.random(coll.length - 1));
}

__.sample = function(coll, sampleSize) {
  var i, memo = [], randomIndex;

  if (__.isUndefined(sampleSize)) {
    randomIndex = Math.floor(Math.random(coll.length - 1));
    return coll[randomIndex];
  }

  for (i = sampleSize; i > 0; i--) {
    randomIndex = Math.floor(Math.random(coll.length - 1));
    memo.push(coll[randomIndex]);
  }
  return memo;
};
